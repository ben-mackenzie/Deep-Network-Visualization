...
class CustomReLU(TorchFunc):
    ...
    def backward(self, y):
        ...
        ##############################################################################
        new_input = self.saved_tensors[0]
        grad_input = y.clone()
        grad_input[grad_input < 0] = 0
        grad_input[new_input < 0] = 0
        return grad_input
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        scores_true = scores.gather(1, y_tensor.view(-1, 1)).squeeze()
        scores_true.backward(torch.ones(scores_true.shape), retain_graph=True)
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        scores_true = scores.gather(1, y_tensor.view(-1, 1)).squeeze()
        scores_true.backward(torch.ones(scores_true.shape), retain_graph=True)
        ack = torch.mean(self.gradient_value, dim=(2,3))
        lin_comb = torch.sum(ack.unsqueeze(2).unsqueeze(3) * self.activation_value, dim=1)
        cam = torch.clamp(lin_comb, min=0).detach().numpy()
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
