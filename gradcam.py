...
from captum.attr import GuidedGradCam, GuidedBackprop, LayerAttribution
...
visualize_attr_maps('visualization/guided_backprop_captum.png', X, y, class_names, [attr_gbp], ['Guided BackProp'])

# Computing Guided GradCam
ggc = GuidedGradCam(model, conv_module)
attr_ggc = compute_attributions(ggc, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/guided_gradcam_captum.png', X, y, class_names, [attr_gbp], ['Guided GradCam'])
##############################################################################
#                             END OF YOUR CODE                               #
##############################################################################
...
layer_act_attr = compute_attributions(layer_act, X_tensor)
layer_act_attr_sum = layer_act_attr.mean(axis=1, keepdim=True)
layer_act_2 = LayerConductance(model, layer)
layer_act_attr_2 = compute_attributions(layer_act_2, X_tensor, target=y_tensor)
layer_act_attr_sum_2 = layer_act_attr_2.mean(axis=1, keepdim=True)
layer_act_3 = LayerGradCam(model, layer)
layer_act_attr_3 = compute_attributions(layer_act_3, X_tensor, target=y_tensor)
layer_act_attr_sum_3 = layer_act_attr_3.mean(axis=1, keepdim=True)

N, C, H, W = X_tensor.shape
interp = LayerAttribution.interpolate(layer_act_attr_sum,(H,W))
img = np.concatenate((interp, interp, interp), 1)
img = torch.from_numpy(img)
visualize_attr_maps('visualization/layer_activation.png', X, y, class_names, [img], ['Layer Activation'])
interp_2 = LayerAttribution.interpolate(layer_act_attr_sum_2,(H,W)).detach().numpy()
img_2 = np.concatenate((interp_2, interp_2, interp_2), 1)
img_2 = torch.from_numpy(img_2)
visualize_attr_maps('visualization/layer_conductance.png', X, y, class_names, [img_2], ['Layer Conductance'])
interp_3 = LayerAttribution.interpolate(layer_act_attr_sum_3,(H,W)).detach().numpy()
img_3 = np.concatenate((interp_3, interp_3, interp_3), 1)
img_3 = torch.from_numpy(img_3)
visualize_attr_maps('visualization/layer_gradcam.png', X, y, class_names, [img_3], ['Layer GradCam'])
##############################################################################
#                             END OF YOUR CODE                               #
##############################################################################
